...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="record">
    <transformedRecord>
      ...
      <xsl:element name="CODART"><xsl:value-of select="format-number(*[local-name()='MATNR'], '0000000000')"/></xsl:element>
      ...
      <xsl:element name="DISTR_CHAN"><xsl:value-of select="string(*[local-name()='VTWEG'])"/></xsl:element>
    </transformedRecord>
  </xsl:template>
</xsl:stylesheet>